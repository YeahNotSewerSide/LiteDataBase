...
public:
	...
	void set_value(unsigned char* value,char* type) {
		...
		if (strcmp(type, "integer\0") == 0 || strcmp(type, "uinteger\0") == 0 || strcmp(type, "float\0") == 0) {
			if (empty) {
				this->value = new unsigned char[sizeof(int)];
			}
			memcpy(this->value, value, sizeof(int));
		}
		if (strcmp(type, "long\0") == 0 || strcmp(type, "ulong\0") == 0 || strcmp(type, "double\0") == 0) {
			if (empty) {
				this->value = new unsigned char[sizeof(long)];
			}
			memcpy(this->value, value, sizeof(long));
		}
		if (strcmp(type, "boolean\0") == 0) {
			if (empty) {
...
int main()
{
	...
	input = 999;
	unsigned char* num1 = new unsigned char[sizeof(int)];
	num1[0] = input & 0xFF;
	num1[1] = (input >> 8) & 0xFF;
	num1[2] = (input >> 16) & 0xFF;
	num1[3] = (input >> 24) & 0xFF;
	DB db(name,4,5000000);
	db.init_column(0, (char*)"First\0", (char*)"integer\0");
	...
	db.set_value((char*)"First\0", 1, num1);
	...
	////db.clear_db();
	int c = *(int*)db.get_value((char*)"First\0", 1);
	//592
	std::cout << std::endl;
}
